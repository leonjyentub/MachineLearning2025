...
class Dataset():
    def __init__(self, path, sequence_length):
        ...
        corpus = cc.convert(open(path, 'r', encoding='UTF-8').read()
                            ).replace('\n', '').replace(' ', '').replace('　', '')
    ...
    def __getitem__(self, index):
        return (
            torch.tensor(
                self.words_indexes[index:index + self.sequence_length]).to(device),
            torch.tensor(
                [self.words_indexes[index + self.sequence_length]]).to(device),
        )
...
def predict(dataset, model, text, next_words=100):
    ...
    for i in range(0, next_words):
        x = torch.tensor([dataset.word_to_index[w]
                         for w in text[i:i+100]]).to(device)
        y_pred = model(x)[-1]
